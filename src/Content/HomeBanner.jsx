...
const HomeBanner = () => {
  ...
  const allTitles = TeEn.stories.flatMap((each) => each.bannerImge);
  const allNames = TeEn.stories.flatMap((each) => each.name);
        ...
        {allTitles.map((eachTitle, index) => (
          <SwiperSlide key={index}>
            <div
              ...
              className="relative lg:h-[55vh] md:h-[50vh] h-[35vh] w-full bg-cover bg-center bg-no-repeat cursor-pointer rounded-lg"
